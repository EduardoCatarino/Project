...
/// @notice It inherits the Storage contract to not have collition with vault
contract Vault is Storage{
    ...
    function initialize(address _LPToken, address _theToken) public {
    ...
    function deposit(uint _amount, uint _lockingPeriod) external returns(uint){
        ...
        ids++;
        totalFunds += _amount * _lockingPeriod;
    ...
    function checkLocks() public {
        ...
        for(x=locks.length; x>0; x--){
            if(locks[x-1].endTime <= block.timestamp){
                ...
                totalFunds -= deposits[locks[x-1].user][locks[x-1].id].amount * deposits[locks[x-1].user][locks[x-1].id].lockingPeriod;
